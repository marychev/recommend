...
async def cache_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка проверки кэша: {str(e)}"
        )
...
async def cache_keys():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка получения ключей: {str(e)}"
        )
...
async def test_cache_operations():
    ...
    try:
        ...
        results["cache_save"] = {"success": save_result, "time_ms": save_time}

        # Получение из кэша
        get_start = time.time()
        cached_data = await get_cached_recommendations(
            user_id=test_user_id, top_n=10, exclude_listened=True
        )
        ...
        results["cache_key"] = {"key": cache_key, "exists": key_exists}
...
async def set_cache_ttl(ttl_hours: int):
    ...
    if ttl_hours < 1 or ttl_hours > 24:
        raise HTTPException(
            status_code=400, detail="TTL должен быть от 1 до 24 часов"
        )
...
async def get_active_users_for_warmup():
    ...
    active_users = await warmup_service.get_active_users(
        hours_back=24, min_interactions=3, limit=20
    )

    return {
        "active_users": active_users,
        "count": len(active_users),
        "criteria": {"hours_back": 24, "min_interactions": 3, "limit": 20},
        "timestamp": datetime.now(),
    }
...
async def simulate_cache_hitrate():
    ...
    try:
        ...
        results["avg_cache_time"] = (
            sum(cache_times) / len(cache_times) if cache_times else 0
        )
        results["avg_miss_time"] = (
            sum(miss_times) / len(miss_times) if miss_times else 0
        )
...
async def clear_user_cache(user_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка очистки кэша: {e}"
        )
...
async def clear_all_cache():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка очистки кэша: {str(e)}"
        )
...
async def test_real_scenario_v2():
    ...
    try:
        ...
        cache_hit1 = (
            resp1.performance_metrics.cache_hit
            if resp1.performance_metrics
            else False
        )
        ...
        cache_hit2 = (
            resp2.performance_metrics.cache_hit
            if resp2.performance_metrics
            else False
        )
        results["scenario_steps"].append(
            {
        ...
        results["invalidation_log"].append(
            "Событие PLAY - кэш НЕ должен инвалидироваться"
        )
        ...
        cache_hit3 = (
            resp3.performance_metrics.cache_hit
            if resp3.performance_metrics
            else False
        )
        results["scenario_steps"].append(
            {
        ...
        cache_hit4 = (
            resp4.performance_metrics.cache_hit
            if resp4.performance_metrics
            else False
        )
        results["scenario_steps"].append(
            {
        ...
        cache_hit5 = (
            resp5.performance_metrics.cache_hit
            if resp5.performance_metrics
            else False
        )
        results["scenario_steps"].append(
            {
        ...
        results["final_hit_rate"] = (
            (results["cache_hits"] / total_requests * 100)
            if total_requests > 0
            else 0
        )
...
async def test_real_scenario():
    ...
    try:
        ...
        def _fill_dict_(
            step: int, action: str, cache_hit: bool, time_ms: float
        ):
            return {
                "step": step,
                "action": action,
        ...
        results["scenario_steps"].append(
            _fill_dict_(1, "Первый запрос рекомендаций", cache_hit1, time1)
        )
        ...
        results["scenario_steps"].append(
            _fill_dict_(
                2,
                "Второй запрос рекомендаций (ожидается cache hit)",
                cache_hit2,
                time2,
            )
        )
        ...
        if play_event.action_type not in [
            ActionType.LIKE,
            ActionType.DISLIKE,
        ...
        results["scenario_steps"].append(
            _fill_dict_(
                3,
                "После события PLAY (ожидается cache hit)",
                cache_hit3,
                time3,
            )
        )
        ...
        like_event = UserTrackInteractionCreate(
            user_id=test_user_id, track_id=2, action_type=ActionType.LIKE
        )
        background_tasks = BackgroundTasks()
        ...
        results["scenario_steps"].append(
            _fill_dict_(
                4,
                "После события LIKE (ожидается cache miss)",
                cache_hit4,
                time4,
            )
        )
        ...
        results["scenario_steps"].append(
            _fill_dict_(
                5, "Пятый запрос (ожидается cache hit)", cache_hit5, time5
            )
        )
