class TestDBSchema:
    ...
    async def test_users_table_structure(
        self, clickhouse_client, create_test_schema
    ):
        """Тест структуры таблицы users"""
        result = await clickhouse_client.execute_raw("DESCRIBE TABLE users")
    ...
    async def test_tracks_table_structure(
        self, clickhouse_client, create_test_schema
    ):
        """Тест структуры таблицы tracks"""
        result = await clickhouse_client.execute_raw("DESCRIBE TABLE tracks")
