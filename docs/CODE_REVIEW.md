...
### 4. Дублирование буферизации

**Файлы**: `app/db/clickhouse.py`, `app/kafka/data_handler.py`

Буфер с батчингом реализован **дважды**:
- `ClickHouseClient._insert_buffer` (в `clickhouse.py`)
- `KafkaDataHandler._buffers` (в `data_handler.py`)

**Проблема**: Избыточность и путаница.

**Решение**: Оставить один механизм батчинга, убрать дублирование.
...
| 4 | 🟠 Средний | Дублирование буферизации | ⏳ TODO |
