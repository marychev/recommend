...
═══════════════════════════════════════════════════════════
📊 Общая статистика:                         
• Виртуальных пользователей: 10     | 10     | 10   
• Всего запросов:            1250   | 1300   | 1255 
• Общий процент ошибок:      59.76% | 59.76% | 59.77%        
...
❌ Высокий процент ошибок  59.76% || 59.77% || 59.76%
...
📊 Статистика:                                 
   Всего запросов:        84        |         
   Среднее время ответа:  29.81ms   | 28.69     |   
   95 перцентиль:         64.46ms   | 84.120ms  | 84.20ms 
   Процент ошибок:        0.00%     
   Успешные проверки:     100.00%   

✅ PASSED: API работает нормально. Можно запускать полноценные тесты!  source=console
...
• Всего запросов:        4453     || 4508        || 4465 
• Среднее время:         242.78ms || 235.26ms    || 240.12ms
• 95 перцентиль:         630.53ms || 727.85ms    || 665.08ms
• 99 перцентиль:         0.00ms      
• Процент ошибок:        60.61%   || 59.49%      || 59.08%

❌ FAILED: Слишком много ошибок при пиковой нагрузке. Требуется оптимизация!
...
User 49687669: ❌ Error 404                    source=console
User 13900344: ❌ Error 404                    source=console
User 97117707: ❌ Error 404                    source=console
User 44356456: ❌ Error 404                    source=console
User 20204459: ❌ Error 404                    source=console
User 30310319: ❌ Error 404                    source=console
User 18045405: ❌ Error 404                    source=console
User 87112654: ❌ Error 404                    source=console
User 35079639: ❌ Error 404                    source=console
User 43982948: ❌ Error 404                    

═══════════════════════════════════════════════════════════════════════════════
   ⚡ БЫСТРЫЙ ТЕСТ ПРОИЗВОДИТЕЛЬНОСТИ РЕКОМЕНДАЦИЙ
═══════════════════════════════════════════════════════════════════════════════
INFO[0000] 📊 Обработано запросов: 10                     
───────────────────────────────────────────────────────────────────────────────
💾 КЭШ:                                        
• Попадания в кэш:  0                      
• Промахи кэша:     0                      
...
• Успешных запросов:          0            
• Ошибок:                     20           
• Среднее время HTTP:         28.76ms   || 22.36ms   || 45.79ms
• p95 HTTP:                   52.03ms   || 31.17ms   || 127.97ms  
...
# [ERROR] k6 run load_tests k6_recommendations_performance_test.js
...
════════════════════════════════════════════════════════════════════════════════  
📊 Общая статистика:                           
• Длительность теста:    5m 0s             
• Виртуальных юзеров:    50                
• Всего запросов:        5072    || 5205     || 5261 
• RPS (req/sec):         16.86   || 17.48    || 17.29 
• Процент ошибок:        99.94%  || 99.83%   || 99.90%     
                                               
⏱️  Время ответа:                             
• Минимум:               12ms    || 11ms              
• Среднее:               66ms    || 28ms  || 38ms 
• Медиана:               33ms    || 18ms              
• 95 перцентиль:         194ms   || 77ms  || 135ms
• 99 перцентиль:         0ms               
• Максимум:              1847ms  || 546ms || 2754ms
...
• Похожих пользователей (среднее): 12.3   || 16.8   
• Похожих пользователей (мин):     11     || 9     || 6
• Похожих пользователей (макс):    14     || 28    || 14
...
📈 АНАЛИЗ РАСПРЕДЕЛЕНИЯ ВРЕМЕНИ (среднее):     
   • Redis:                   9.00ms (2.2%)      || 10.20ms (2.7%)  
   • ClickHouse:              403.56ms (97.7%)   || 366.08ms (97.1%)  
   • Алгоритм:                0.42ms (0.1%)      || 0.53ms (0.1%)      
   • Прочее (сеть, FastAPI):  0.14ms (0.0%)      || 0.12ms (0.0%)      
   • ИТОГО:                   413.13ms (100.0%)  || 376.94ms (100.0%)  
   ---------------------------------------------------------------
   • Redis:                   11.14ms (0.9%)  
...
💡 РЕКОМЕНДАЦИИ ПО ОПТИМИЗАЦИИ:                source=console
⚠️  Низкий Hit Rate кэша (<30%). Рассмотрите увеличение TTL или предварительный прогрев кэша.
